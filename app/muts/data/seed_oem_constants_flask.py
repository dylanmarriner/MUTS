...
Run with: flask shell < seed_oem_constants_flask.py
"""

from muts.models.database_models import TransmissionType, DrivetrainType
...
def create_holden_vf_constants():
    ...
    db.session.commit()
    print(f"Created Holden VF constants preset: {constants.id}")
    return constants
...
def create_vw_golf_constants():
    ...
    db.session.add(constants)
    db.session.commit()
    print(f"Created VW Golf constants preset: {constants.id}")
    return constants
...
def create_vehicle_profiles(holden_preset, vw_preset):
    ...
    db.session.add(holden_constants)
    ...
    db.session.add(vw_constants)
    db.session.commit()
    
    print(f"Created vehicle profiles: Holden {holden_profile.id}, VW {vw_profile.id}")
...
def create_all():
    ...
    try:
        ...
        print(f"Failed to create OEM constants: {e}")
        db.session.rollback()
        return False
