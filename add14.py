...
class TwoStepRevLimiter:
    ...
    def calculate_rev_limit_parameters(self, engine_rpm: float, vehicle_speed: float,
        ...
        # Normal redline protection
            ...
            parameters['ignition_retard'] = -rpm_over * 0.5  # 0.5° retard per RPM over
    ...
    def _ignition_cut_strategy(self, current_rpm: float, target_rpm: float) -> Dict:
        ...
        if rpm_error > 0:
            ...
            if np.random.random() < cut_probability:
                # Cut random cylinder(s)
                cylinders_to_cut = np.random.choice([1, 2, 3, 4], 
                                                   size=min(2, int(cut_probability * 4)), 
                                                   replace=False)
                cut_parameters['cut_cylinders'] = cylinders_to_cut.tolist()
